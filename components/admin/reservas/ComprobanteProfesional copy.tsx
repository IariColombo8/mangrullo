...
const ComprobanteProfesional: React.FC<ComprobanteProfesionalProps> = ({
  ...
  useEffect(() => {
    const calculateScale = () => {
      ...
      const comprobanteWidth = 794;
      const comprobanteHeight = 1100; // Altura aproximada del comprobante
      const buttonHeight = 80; // Espacio para botones + padding
      const padding = 32; // Padding total del contenedor

      // Calcular escala basada en ancho y alto del viewport
      const availableWidth = viewportWidth - padding;
      const availableHeight = viewportHeight - buttonHeight - padding;

      const scaleByWidth = availableWidth / comprobanteWidth;
      const scaleByHeight = availableHeight / comprobanteHeight;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = `comprobante-${reserva.nombre.replace(
        /\s+/g,
        "-"
      )}-${format(new Date(), "dd-MM-yyyy")}.png`;
    ...
    <div className="w-full h-full flex flex-col items-center justify-center overflow-hidden">
      <div className="w-full flex flex-col items-center justify-center overflow-hidden">
        <div
          ...
          className="bg-white border-4 border-double border-gray-400 rounded-lg p-6 shadow-2xl origin-center"
          style={{
            width: "794px",
            height: "1100px",
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 flex gap-2 sm:gap-3 justify-center shadow-lg z-10">
          <div className="flex gap-2 sm:gap-3 w-full max-w-2xl">
            <Button
              onClick={onClose}
              variant="outline"
              className="flex-1 border-gray-300 hover:bg-gray-100 bg-transparent text-xs sm:text-sm"
            >
              Cerrar
            </Button>
            <Button
              onClick={handleDownload}
              className="flex-1 bg-blue-600 hover:bg-blue-700 text-white shadow-md text-xs sm:text-sm"
            >
            ...
            <Button
              onClick={onEdit}
              className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white shadow-md text-xs sm:text-sm"
            >
