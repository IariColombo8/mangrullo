                  ...
                  <CardContent className="p-3 md:p-6 pt-0">
                    ...
                    <div className="flex flex-col gap-1.5 md:gap-2">
                      <Button
                        onClick={() => handleViewDetails(cabin)}
                        variant="outline"
                        className="w-full bg-emerald-600 hover:bg-black-700 text-[10px] md:text-sm h-7 md:h-10"
                      >
                        Ver Detalles
                      </Button>
                      <Button
                        ...
                        className="w-full border-white-10 text-black-700 text-[10px] md:text-sm h-7 md:h-10"
                      >
        ...
        {selectedCabin && (
          <Dialog open={!!selectedCabin} onOpenChange={handleCloseModal}>
            <DialogContent className="max-w-10xl max-h-[90vh] overflow-y-auto p-0">
              <DialogHeader className="p-2 md:p-9 pb-0">
                <DialogTitle className="text-xl md:text-2xl font-bold flex items-center gap-2 md:gap-3 pr-8">
              ...
              <div className="px-4 md:px-6 pb-4 md:pb-6">
                ...
                {Array.isArray(selectedCabin.images) && selectedCabin.images.length > 0 && (
                  <div className="mb-4 md:mb-6">
                    <div className="relative h-48 md:h-96 rounded-lg overflow-hidden bg-gray-100">
                      <Image
                      ...
                      {selectedCabin.images.length > 1 && (
                        <>
                          <button
                            ...
                            className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full p-1.5 md:p-2 transition-all"
                          >
                            <ChevronLeft className="w-4 h-4 md:w-6 md:h-6" />
                          </button>
                          <button
                            onClick={handleNextImage}
                            className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full p-1.5 md:p-2 transition-all"
                          >
                            <ChevronRight className="w-4 h-4 md:w-6 md:h-6" />
                          </button>
                          <div className="absolute bottom-3 md:bottom-4 left-1/2 -translate-x-1/2 bg-black/70 text-white px-2 md:px-3 py-1 rounded-full text-xs md:text-sm">
                      ...
                      <div className="flex gap-2 mt-3 md:mt-4 overflow-x-auto pb-2">
                        {selectedCabin.images.map((img, idx) => (
                          <button
                            ...
                            className={`relative h-12 w-12 md:h-16 md:w-16 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${
                ...
                <div className="mb-4 md:mb-6">
                  ...
                  <p className="text-sm md:text-base text-gray-700">{getCabinDescription(selectedCabin)}</p>
                </div>

                {/* Capacidad */}
                <div className="flex items-center gap-2 mb-4 md:mb-6 text-emerald-700 bg-emerald-50 p-3 md:p-4 rounded-lg">
                  <Users className="w-4 h-4 md:w-5 md:h-5" />
                  <span className="text-sm md:text-base font-medium">Capacidad: Hasta {selectedCabin.capacity} personas</span>
                </div>

                {/* Amenidades */}
                {getActiveAmenities(selectedCabin).length > 0 && (
                  <div className="mb-4 md:mb-6">
                    <h3 className="text-base md:text-lg font-semibold text-gray-900 mb-3 md:mb-4">El departamento cuenta con:</h3>

                    {/* Vista y Espacios */}
                    {getActiveAmenities(selectedCabin).includes('balconyView') && (
                      <div className="mb-3 md:mb-4">
                        <h4 className="text-xs md:text-sm font-semibold text-gray-700 mb-2">Vista y Espacios</h4>
                        <div className="flex items-center gap-2 bg-blue-50 p-2 md:p-3 rounded-lg">
                    ...
                    {getActiveAmenities(selectedCabin).filter(a =>
                      ...
                      <div className="mb-3 md:mb-4">
                        <h4 className="text-xs md:text-sm font-semibold text-gray-700 mb-2">Equipamiento Completo</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {['ac', 'fridge', 'microwave', 'kettle', 'electricPot', 'dishes', 'waterHeater', 'tv', 'wifi']
                            ...
                            .map((amenity) => {
                              const info = getAmenityInfo(amenity)
                              return (
                                <div key={amenity} className="flex items-center gap-2 bg-emerald-50 p-2 md:p-3 rounded-lg">
                    ...
                    {getActiveAmenities(selectedCabin).filter(a =>
                      ...
                      <div className="mb-3 md:mb-4">
                        <h4 className="text-xs md:text-sm font-semibold text-gray-700 mb-2">Ropa de Cama Incluida</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          {['bedding', 'blankets', 'towels']
                            ...
                            .map((amenity) => {
                              const info = getAmenityInfo(amenity)
                              return (
                                <div key={amenity} className="flex items-center gap-2 bg-purple-50 p-2 md:p-3 rounded-lg">
                                  <Check className="w-4 h-4 md:w-5 md:h-5 text-purple-600" />
                                  <span className="text-xs md:text-sm">{info.label} {info.emoji}</span>
                                </div>
                              )
                            })}
                ...
                <div className="pt-2 md:pt-4">
                  <Button
                    ...
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-sm md:text-base h-10 md:h-12"
                  >
